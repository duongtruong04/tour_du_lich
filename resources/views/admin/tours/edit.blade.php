            ...
            <div class="lg:col-span-2 space-y-8">
                <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="number" name="base_price" required value="{{ old('base_price', $tour->base_price) }}"
                                min="1" step="1"
                                class="w-full px-5 py-4 bg-slate-50 border-0 rounded-2xl focus:ring-2 focus:ring-teal-500 transition-all font-bold text-slate-700 placeholder-slate-300">
                            @error('base_price')
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="duration" required value="{{ old('duration', $tour->duration) }}" placeholder="VD: 3 ngày 2 đêm"
                                class="w-full px-5 py-4 bg-slate-50 border-0 rounded-2xl focus:ring-2 focus:ring-teal-500 transition-all font-bold text-slate-700 placeholder-slate-300">
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        @php $currentTransportation = old('transportation', $tour->transportation); @endphp
                        <select name="transportation" required
                            ...
                            <option value="Máy bay" {{ $currentTransportation == 'Máy bay' ? 'selected' : '' }}>✈️ Máy bay</option>
                            <option value="Du thuyền" {{ $currentTransportation == 'Du thuyền' ? 'selected' : '' }}>🚢 Du thuyền</option>
                            <option value="Xe du lịch chất lượng cao" {{ $currentTransportation == 'Xe du lịch chất lượng cao' ? 'selected' : '' }}>🚌 Xe du lịch chất lượng cao</option>
                        </select>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="summary" rows="3"
                            ...
                            class="w-full px-5 py-4 bg-slate-50 border-0 rounded-2xl focus:ring-2 focus:ring-teal-500 transition-all font-medium text-slate-700 placeholder-slate-300">{{ old('summary', $tour->summary) }}</textarea>
                        @error('summary')
                            <p class="mt-2 text-[10px] font-bold text-rose-500 uppercase tracking-widest px-2">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="google_map" rows="3"
                            ...
                            class="w-full px-5 py-4 bg-slate-50 border-0 rounded-2xl focus:ring-2 focus:ring-teal-500 transition-all font-medium text-slate-700 placeholder-slate-300">{{ old('google_map', $tour->google_map) }}</textarea>
                        @error('google_map')
                            <p class="mt-2 text-[10px] font-bold text-rose-500 uppercase tracking-widest px-2">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="itinerary" class="editor" placeholder="Nhập lịch trình chi tiết từng ngày...">{{ old('itinerary', $tour->itinerary) }}</textarea>
                        @error('itinerary')
                            <p class="mt-2 text-[10px] font-bold text-rose-500 uppercase tracking-widest px-2">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div class="space-y-2">
                            ...
                            <textarea name="service_includes" class="editor" placeholder="VD: Vé máy bay, Khách sạn 4 sao, Ăn uống...">{{ old('service_includes', $tour->service_includes) }}</textarea>
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea name="service_excludes" class="editor" placeholder="VD: VAT, Chi phí cá nhân, Tip HDV...">{{ old('service_excludes', $tour->service_excludes) }}</textarea>
                ...
                <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4" id="image-gallery">
                        ...
                        <div class="aspect-square rounded-3xl bg-slate-50 overflow-hidden shadow-sm relative group">
                            <img src="{{ $image->image_url }}" class="w-full h-full object-cover">
                            <div class="absolute inset-0 bg-teal-950/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                <button type="button" onclick="confirmDeleteImage({{ $image->id }})" class="w-10 h-10 bg-rose-500 text-white rounded-xl flex items-center justify-center hover:bg-rose-600 transition-colors">
                    ...
                    <p class="mt-4 text-[10px] text-slate-400 font-bold italic leading-relaxed">Bạn có thể xóa ảnh cũ hoặc tải lên thêm ảnh mới.</p>
                </div>
            </div>
            ...
            <div class="space-y-8">
                    ...
                    <div class="space-y-3 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
                        ...
                        <label class="flex items-center group cursor-pointer p-2 hover:bg-teal-50 rounded-xl transition-colors">
                            <input type="checkbox" name="destinations[]" value="{{ $dest->id }}" 
                                {{ in_array($dest->id, $tour->destinations->pluck('id')->toArray()) ? 'checked' : '' }}
                                class="w-5 h-5 rounded-lg border-slate-200 text-teal-600 focus:ring-teal-500 cursor-pointer">
                ...
                <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
                    ...
                    <select name="is_active" class="w-full px-5 py-4 bg-slate-50 border-0 rounded-2xl focus:ring-2 focus:ring-teal-500 transition-all font-bold text-slate-700">
                        <option value="1" {{ $tour->is_active ? 'selected' : '' }}>Hiển thị website</option>
                        <option value="0" {{ !$tour->is_active ? 'selected' : '' }}>Tạm ẩn</option>
                    </select>
                </div>
                ...
                <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
                    <h2 class="text-xs font-black text-teal-900 uppercase tracking-widest mb-6 flex items-center">
                        ...
                        Ngày khởi hành ({{ $tour->departures->count() }})
                    </h2>
                    <div id="departures-container" class="space-y-4">
                        @foreach($tour->departures as $index => $dep)
                        <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-3" id="departure-row-existing-{{ $index }}">
                            <div class="flex items-center justify-between">
                                <span class="text-[10px] font-black text-teal-600 uppercase tracking-widest">Ngày khởi hành #{{ $index + 1 }}</span>
                                <button type="button" onclick="removeDepartureRow('departure-row-existing-{{ $index }}')" class="text-rose-400 hover:text-rose-600 transition-colors">
                                    <i class="fas fa-times"></i>
                                </button>
                            </div>
                            <div class="grid grid-cols-1 gap-3">
                                <input type="hidden" name="departures[{{ $index }}][id]" value="{{ $dep->id }}">
                                <div>
                                    <label class="block text-[10px] font-bold text-slate-500 mb-1 ml-1">Ngày khởi hành <span class="text-rose-500">*</span></label>
                                    <input type="date" name="departures[{{ $index }}][start_date]" value="{{ $dep->start_date->format('Y-m-d') }}" required class="w-full px-4 py-2.5 bg-white border-0 rounded-xl focus:ring-2 focus:ring-teal-500 text-sm font-bold text-slate-700">
                                </div>
                                <div>
                                    <label class="block text-[10px] font-bold text-slate-500 mb-1 ml-1">Số chỗ tối đa</label>
                                    <input type="number" name="departures[{{ $index }}][max_seats]" value="{{ $dep->max_seats }}" min="1" class="w-full px-4 py-2.5 bg-white border-0 rounded-xl focus:ring-2 focus:ring-teal-500 text-sm font-bold text-slate-700">
                                </div>
                                <div>
                                    <label class="block text-[10px] font-bold text-slate-500 mb-1 ml-1">Giá riêng (để trống = giá cơ bản)</label>
                                    <input type="number" name="departures[{{ $index }}][price_override]" value="{{ $dep->price_override }}" placeholder="Giá riêng nếu có" class="w-full px-4 py-2.5 bg-white border-0 rounded-xl focus:ring-2 focus:ring-teal-500 text-sm font-bold text-slate-700 placeholder-slate-300">
                                </div>
                            </div>
                        </div>
                        @endforeach
                    ...
                    <p class="mt-3 text-[10px] text-slate-400 font-bold italic leading-relaxed">Lưu ý: Khi cập nhật, toàn bộ ngày khởi hành sẽ được lưu lại theo danh sách hiện tại.</p>
                </div>
...
<script>
    ...
    function confirmDeleteImage(imageId) {
        if (confirm('Xác nhận xóa ảnh này khỏi tour?')) {
            const form = document.getElementById('delete-image-form');
            form.action = `/admin/tours/images/${imageId}`;
            form.submit();
        }
    }
    ...
    function addDepartureRow() {
        ...
        row.id = 'departure-row-new-' + departureIndex;
        row.innerHTML = `
            <div class="flex items-center justify-between">
                <span class="text-[10px] font-black text-teal-600 uppercase tracking-widest"><i class="fas fa-plus-circle mr-1"></i> Ngày khởi hành mới</span>
                <button type="button" onclick="removeDepartureRow('departure-row-new-${departureIndex}')" class="text-rose-400 hover:text-rose-600 transition-colors">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <div class="grid grid-cols-1 gap-3">
