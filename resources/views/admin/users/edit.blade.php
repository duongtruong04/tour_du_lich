        ...
        <form action="{{ isset($user) ? route('admin.users.update', $user) : route('admin.users.store') }}" method="POST" enctype="multipart/form-data" class="p-10 space-y-10">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                ...
                <div class="form-group">
                    ...
                    @error('full_name')
                        <p class="mt-2 text-[10px] font-bold text-rose-500 uppercase tracking-widest animate-fade-in px-2">{{ $message }}</p>
                    @enderror
                </div>

                <div class="form-group">
                    <label class="form-label">Email đăng nhập <span class="text-rose-500">*</span></label>
                    <input type="email" name="email" value="{{ old('email', $user->email ?? '') }}" required placeholder="admin@example.com" class="form-control">
                    @error('email')
                        <p class="mt-2 text-[10px] font-bold text-rose-500 uppercase tracking-widest animate-fade-in px-2">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="password" name="password" {{ isset($user) ? '' : 'required' }} placeholder="••••••••" class="form-control">
                    @error('password')
                        <p class="mt-2 text-[10px] font-bold text-rose-500 uppercase tracking-widest animate-fade-in px-2">{{ $message }}</p>
                    @enderror
                </div>
