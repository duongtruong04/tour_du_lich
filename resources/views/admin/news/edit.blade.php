...
<div class="max-w-5xl mx-auto">
    <div class="flex items-center justify-between mb-8">
        <h1 class="text-3xl font-black text-slate-800 tracking-tight">Chỉnh sửa <span class="text-teal-600">Bài viết</span></h1>
    ...
    <form id="news-form" action="{{ route('admin.news.update', $news) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
                ...
                <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="content" id="news-content" rows="12"
                            class="w-full px-5 py-4 bg-slate-50 border-0 rounded-2xl focus:ring-2 focus:ring-teal-500 transition-all font-medium text-slate-700 placeholder-slate-300">{{ old('content', $news->content) }}</textarea>
                        @error('content')
                            <p class="mt-2 text-[10px] font-bold text-rose-500 uppercase tracking-widest px-2">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="summary" rows="3"
                            class="w-full px-5 py-4 bg-slate-50 border-0 rounded-2xl focus:ring-2 focus:ring-teal-500 transition-all font-medium text-slate-700 placeholder-slate-300">{{ old('summary', $news->summary) }}</textarea>
                        @error('summary')
                            <p class="mt-2 text-[10px] font-bold text-rose-500 uppercase tracking-widest px-2">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="space-y-8">
                ...
                <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
                    ...
                    <div class="relative group">
                        <div id="image-preview" class="w-full aspect-[16/9] rounded-2xl bg-slate-50 border-2 border-dashed border-slate-200 flex items-center justify-center overflow-hidden transition-all group-hover:border-teal-400 group-hover:bg-teal-50 shadow-inner">
                            @if($news->image_path)
                    ...
                    <p class="mt-4 text-[10px] text-slate-400 font-bold italic leading-relaxed text-center">Nhấn vào ảnh để thay đổi mới.</p>
                    @error('image')
                        <p class="mt-2 text-[10px] font-bold text-rose-500 uppercase tracking-widest px-2">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="pt-4 flex flex-col gap-4">
                    ...
                    <button type="button" onclick="history.back()" class="w-full py-5 bg-slate-100 text-slate-400 rounded-[1.5rem] font-black text-xs uppercase tracking-widest hover:bg-slate-200 transition-all">Quay lại</button>
                </div>
            </div>
        </div>
    </form>
